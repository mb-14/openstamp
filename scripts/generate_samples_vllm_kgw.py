...
# Get absolute path to the project root (os-watermarking)
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
# Add the project root and MarkLLM to sys.path
...
def filter_length(example, min_length, data_field, tokenizer):
    return len(tokenizer(example[data_field], truncation=True, max_length=min_length)["input_ids"]) >= min_length

def encode(examples, min_length, data_field, tokenizer, device):
...
def evaluate_model(args):
    ...
    prompt_dataset = prompt_dataset.filter(filter_length, fn_kwargs={"min_length": min_length, "tokenizer": tokenizer, "data_field": args.data_field})
    prompt_dataset = prompt_dataset.map(encode, fn_kwargs={"min_length": min_length, "tokenizer": tokenizer, "device": "cuda", "data_field": args.data_field}, batched=True)
