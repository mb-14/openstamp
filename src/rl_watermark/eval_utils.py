...
def evaluate_detection(actor_model, reward_model, tokenizer, test_dataset, do_sample, save_to=None, paraphraser=None, ppl_model=None, num_tests=100, max_length=300, bsize=4):
    ...
    all_labs = []
    saved_info = []
    print("Sampling enabled: %s"%do_sample)
    ...
    for idx, response in enumerate(tqdm(text_generator(KeyDataset(test_dataset[:num_tests],'prompt'), batch_size=bsize, return_full_text=False), desc="llm gen")):
    # for idx in tqdm(range(num_tests)):
        line = test_dataset[idx]
        llm_gen = response[0]['generated_text']
        prompt_inp = tokenizer(line['prompt'], return_tensors='pt')

        human_inp = tokenizer(line['cont_human'], return_tensors='pt')
