...
import src.rl_watermark.utils as utils
import src.rl_watermark.model_utils as model_utils
import src.rl_watermark.ds_utils as ds_utils
from src.rl_watermark.pretrain_detector import generate_RM_dataset, load_RM_dataset
from src.rl_watermark.eval_utils import evaluate_detection, evaluate_other_model_detection, evaluate_substitution_atk
...
def main(args):
    ...
    if args.eval_baseline is not None:
        ...
        if "llama" in args.model_name:
            actor_model = utils.get_model(args.model_name, model_class=AutoModelForCausalLM).train()
        else:
            actor_model = utils.get_model(args.model_name, model_class=AutoModelForCausalLM).train()
        actor_model = ds_utils.convert_linear_layer_to_lora(actor_model, part_module_name='decoder.layers.', lora_dim=128)
    ...
    with torch.no_grad():
        ...
        all_info = [('std_auc', std_auc), ('fpr@90', fpr_at_90_tpr), ('fpr@99', fpr_at_99_tpr), ('ppl', ppl)]
        print (all_info)
