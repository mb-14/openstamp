...
\[
\mathcal{L}(P) = \frac{1}{|\mathcal{B}|} \sum_{i \in \mathcal{B}} D_{\mathrm{KL}}\!\left( \mathrm{softmax}(\mathbf{s}^P_i / T_{\mathrm{sim}}) \,\middle\|\, \mathrm{softmax}(\mathbf{s}^e_i / T_{\mathrm{sim}}) \right),
\]
...
\[
\mathcal{G}_l = \{\, i \mid \mathrm{PRF}(\texttt{seed}, l, i) < \gamma \,\},
\qquad
